...
def process_experiment(experiment_path: Path, output_path: Path):
    ...
    # Read metrics from all clients
    client_metrics = []
    for client_dir in experiment_path.glob('client_*'):
        if client_dir.is_dir():
            metrics_file = client_dir / 'metrics_history.csv'
            if metrics_file.exists():
                client_df = pd.read_csv(metrics_file)
                client_metrics.append(client_df)
    
    # Calculate aggregated metrics
    all_metrics = pd.concat(client_metrics)
    aggregated_metrics = all_metrics.groupby('round').agg({
        'train_loss': ['mean', 'std'],
        'val_loss': ['mean', 'std'],
        'train_rmse': ['mean', 'std'],
        'val_rmse': ['mean', 'std']
    ...
    # Process each client's data for correlation metrics
    all_client_metrics = {}
    for client_dir in experiment_path.glob('client_*'):
        if client_dir.is_dir():
    ...
    # Calculate average and std of correlation metrics across clients for each round
    rounds = set()
    for client_metrics in all_client_metrics.values():
        rounds.update(client_metrics.keys())
    
    aggregated_corr_metrics = {}
    for round_num in sorted(rounds):
        round_correlations = [
        ...
        aggregated_corr_metrics[round_num] = {
    ...
    with open(metrics_path, 'w') as f:
        json.dump({
            'by_round': aggregated_corr_metrics,
    ...
    # Plot loss with std
    plt.subplot(1, 3, 1)
    plt.errorbar(aggregated_metrics['round'], 
                aggregated_metrics[('train_loss', 'mean')],
                yerr=aggregated_metrics[('train_loss', 'std')],
                fmt='-o', capsize=5, label='Perda de Treino', linewidth=2, color='#2ecc71')
    plt.errorbar(aggregated_metrics['round'], 
                aggregated_metrics[('val_loss', 'mean')],
                yerr=aggregated_metrics[('val_loss', 'std')],
                fmt='-o', capsize=5, label='Perda de Validação', linewidth=2, color='#e74c3c')
    ...
    plt.subplot(1, 3, 2)
    plt.errorbar(aggregated_metrics['round'], 
                aggregated_metrics[('train_rmse', 'mean')],
                yerr=aggregated_metrics[('train_rmse', 'std')],
                fmt='-o', capsize=5, label='RMSE Treino', linewidth=2, color='#2ecc71')
    plt.errorbar(aggregated_metrics['round'], 
                aggregated_metrics[('val_rmse', 'mean')],
                yerr=aggregated_metrics[('val_rmse', 'std')],
                fmt='-o', capsize=5, label='RMSE Validação', linewidth=2, color='#e74c3c')
    ...
    plt.subplot(1, 3, 3)
    rounds = sorted(aggregated_corr_metrics.keys())
    means = [aggregated_corr_metrics[r]['mean_correlation'] for r in rounds]
    stds = [aggregated_corr_metrics[r]['std_correlation'] for r in rounds]
    
    plt.errorbar(rounds, means, yerr=stds, fmt='-o', capsize=5,
                label='Correlação Média entre Clientes', linewidth=2, color='#3498db')
    ...
    aggregated_metrics.to_csv(output_dir / 'processed_history.csv', index=False)

def main():
    # Setup paths
    results_dir = Path('results/fedavg+clusterization')
    output_dir = Path('processed_results/fedavg+clusterization')
